...
// checkNHTSAFields applies hard/soft check logic to a single VIN and its NHTSA golden data.
func checkNHTSAFields(t *testing.T, tdb *sql.DB, vin string, want map[string]string) {
	t.Helper()
...
func TestNHTSAGoldenComparison(t *testing.T) {
	...
	for _, vin := range nhtsaProbeVINs {
		...
		t.Run(vin, func(t *testing.T) {
			checkNHTSAFields(t, tdb, vin, want)
		})
	}
}
...
func TestNHTSALiveComparison(t *testing.T) {
	...
	for _, vin := range nhtsaProbeVINs {
		...
		t.Run(vin, func(t *testing.T) {
			...
			checkNHTSAFields(t, tdb, vin, fields)
		})
	}
}
